/*
  ...
  It temporarily disables foreign key constraints to allow sample data creation,
  then provides instructions for linking to real auth users.

  ## What it creates:
  1. Sample departments
  2. Sample profiles (temporarily without auth constraint)
  3. Medical staff records
  4. Patient records with medical history
  5. Sample medical documents
  6. Sample appointments
  7. Helper functions for linking to real auth users

  ## Important Notes:
  - Creates profiles with placeholder UUIDs
  - Temporarily disables foreign key constraint to auth.users
  - Provides functions to link profiles to real auth users after signup
  - Demo users must be created manually in Supabase Dashboard
*/
...
-- This allows us to create sample profiles without requiring auth users to exist first
...
-- Create helper functions for linking profiles to real auth users

-- Function to link a demo profile to a real auth user
CREATE OR REPLACE FUNCTION link_demo_profile_to_auth_user(demo_email text, auth_user_id uuid)
RETURNS void AS $$
DECLARE
  old_profile_id uuid;
BEGIN
  -- Get the current demo profile ID
  SELECT id INTO old_profile_id FROM profiles WHERE email = demo_email;
  
  IF old_profile_id IS NULL THEN
    RAISE EXCEPTION 'Demo profile with email % not found', demo_email;
  END IF;
  
  -- Update the profile ID to match the auth user ID
  UPDATE profiles SET id = auth_user_id WHERE id = old_profile_id;
  
  -- Update all related tables
  UPDATE medical_staff SET profile_id = auth_user_id WHERE profile_id = old_profile_id;
  UPDATE patients SET profile_id = auth_user_id WHERE profile_id = old_profile_id;
  UPDATE patients SET primary_doctor_id = auth_user_id WHERE primary_doctor_id = old_profile_id;
  UPDATE medical_documents SET patient_id = auth_user_id WHERE patient_id = old_profile_id;
  UPDATE medical_documents SET doctor_id = auth_user_id WHERE doctor_id = old_profile_id;
  UPDATE appointments SET patient_id = auth_user_id WHERE patient_id = old_profile_id;
  UPDATE appointments SET doctor_id = auth_user_id WHERE doctor_id = old_profile_id;
  
  RAISE NOTICE 'Linked demo profile % (%) to auth user %', demo_email, old_profile_id, auth_user_id;
END;
$$ LANGUAGE plpgsql;

-- Function to restore the foreign key constraint after linking profiles
CREATE OR REPLACE FUNCTION restore_profiles_auth_constraint()
RETURNS void AS $$
BEGIN
  ...
  RAISE NOTICE 'Restored foreign key constraint between profiles and auth.users';
END;
$$ LANGUAGE plpgsql;

-- Function to check which demo profiles still need to be linked
CREATE OR REPLACE FUNCTION check_unlinked_demo_profiles()
RETURNS TABLE(email text, profile_id uuid, needs_linking boolean) AS $$
BEGIN
  RETURN QUERY
  SELECT 
    p.email,
    p.id as profile_id,
    NOT EXISTS(SELECT 1 FROM auth.users WHERE auth.users.id = p.id) as needs_linking
  FROM profiles p
  WHERE p.email IN ('patient@demo.com', 'doctor@demo.com', 'admin@demo.com');
END;
$$ LANGUAGE plpgsql;

-- Create a view to easily see demo data status
CREATE OR REPLACE VIEW demo_profiles_status AS
SELECT 
  p.email,
  p.full_name,
  p.role,
  p.id as profile_id,
  CASE 
    WHEN EXISTS(SELECT 1 FROM auth.users WHERE auth.users.id = p.id) 
    THEN 'Linked to auth user'
    ELSE 'Needs linking to auth user'
  END as status
FROM profiles p
WHERE p.email IN ('patient@demo.com', 'doctor@demo.com', 'admin@demo.com');
...
BEGIN
  ...
  RAISE NOTICE '1. Create auth users manually in Supabase Dashboard:';
  RAISE NOTICE '   - patient@demo.com (password: demo123)';
  RAISE NOTICE '   - doctor@demo.com (password: demo123)';
  RAISE NOTICE '   - admin@demo.com (password: demo123)';
  RAISE NOTICE '';
  RAISE NOTICE '2. After creating auth users, link them to demo profiles:';
  RAISE NOTICE '   SELECT link_demo_profile_to_auth_user(''patient@demo.com'', ''real-auth-user-id'');';
  RAISE NOTICE '   SELECT link_demo_profile_to_auth_user(''doctor@demo.com'', ''real-auth-user-id'');';
  RAISE NOTICE '   SELECT link_demo_profile_to_auth_user(''admin@demo.com'', ''real-auth-user-id'');';
  RAISE NOTICE '';
  RAISE NOTICE '3. Check linking status: SELECT * FROM demo_profiles_status;';
  RAISE NOTICE '4. Restore auth constraint: SELECT restore_profiles_auth_constraint();';
END $$;